...
exports.addCategoryById = asyncChoke(async (req, res, next) => {
  ...
  const [approved] = await pool.query(
    `SELECT * FROM restaurants WHERE id = ? AND approved = ?`,
  ...
  const [check] = await pool.query(
    `SELECT * FROM categories WHERE category = ? AND menu_id = ?`,
    [categoryName, menuId]
  );
...
exports.DeleteCategory = asyncChoke(async (req, res, next) => {
  ...
  try {
    ...
    const getcategoryQuery =
      "SELECT id FROM categories WHERE menu_id = ? and id = ?";
    const [getcategories] = await pool.query(getcategoryQuery, [
      menuId,
      categoryId,
    ]);
    ...
    if (getItems.length > 0) {
      ...
      if (deletedItems.affectedRows === 0) {
        return next(
          new AppError(401, "Error while deleting items with this category!")
        );
      }
    }
    ...
    if (categories.affectedRows === 0) {
      return next(
        new AppError(404, "Unable to delete this category, try again later!")
      );
    }
